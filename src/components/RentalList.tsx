...
interface RentalListProps {
  selectedView: string | null; // Define the prop type here
}

const RentalList: React.FC<RentalListProps> = ({ selectedView }) => {
  const navigate = useNavigate();

  // Filter rental houses based on the selected filter
  const filteredHouses = selectedView
    ? rentalHouses.filter(house => house.view === selectedView)
    : rentalHouses; // Use rentalHouses instead of houses

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mt-4 gap-6 p-4 font-serif">
      {filteredHouses.map((house: RentalHouse) => ( // Use filteredHouses here
        <div
          key={house.id}
